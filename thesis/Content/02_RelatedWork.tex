...
Some approaches rely on student records. Nearest-neighbor systems match incoming students to majors based on the historical coursework and grades of students who completed each program \cite{BERTopic}. Skills-based systems compare self-reported interests to a taxonomy of skills associated with each major \cite{BERTopic}. Both require either confidential institutional records or a hand-maintained domain ontology, limiting how broadly they can be applied.

The most relevant prior work is TopProRec, introduced by Hill, Goo, and Agarwal \cite{BERTopic}. TopProRec uses BERTopic to mine interest topics from a university's public course catalog. It converts each course description into an embedding, clusters them into topics, and identifies representative keywords for each cluster. Students select topics that match their interests and a backtracking algorithm ranks programs by how heavily those topics appear in their course offerings. In a case study at a polytechnic with 84 programs, 98\% of 65 participants found the recommendations aligned with their interests.

TopProRec demonstrates that course description content can support program recommendation without collecting student data. MajorMatch works from a similar intuition but operates against a standardized national taxonomy of programs rather than a single institution's course catalog. That shift in scope requires the embeddings to generalize across different ways of describing programs, which is part of what this thesis evaluates.
